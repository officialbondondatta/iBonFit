...
const page = async ({ params }: IExerciseProps) => {
    ...
    try {
        const res = await fetch(`https://api.api-store.workers.dev/api/fitlog/${exerciseid}`)
        if (!res.ok) {
        ...
        <section className=" mt-5 max-w-300 mx-auto lg:p-0 min-h-screen">
            <div className="grid  lg:grid-cols-2  grid-cols-1 lg:gap-20 gap-5 p-5">
                <div className="relative">
                    <Image src={exercise.image} alt={`${exercise.name}`} className="w-full lg:absolute md:h-full object-cover h-auto  rounded-xl" width={600} height={600}></Image>
                </div>
                <div className="">
