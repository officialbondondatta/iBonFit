...
import PlanCards from "../components/my-plan-helper/ExerciseCards/PlanCards";
import SavedCards from "../components/my-plan-helper/ExerciseCards/SavedCards";
                ...
                <div className="mt-10 grid grid-cols-1 gap-5 rounded-2xl">
                    {
                        selectedTab === "plan" ?
                            sortedPlanData.length === 0 ? <NoDataYet></NoDataYet> : sortedPlanData.map((plan) => (
                                <PlanCards key={plan.id} currentPlan={plan}></PlanCards>
                            ))
                            :
                            sortedSavedData.length === 0 ? <NoDataYet></NoDataYet> : sortedSavedData.map((saved) => (
                                <SavedCards key={saved.id} currentPlan={saved}></SavedCards>
